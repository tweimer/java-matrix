    ...
    public SingularValueDecomposition(final Matrix Arg)
    {
        ...
        while (p > 0)
        {
            ...
            else
            {
                ...
                if (ks == k)
                {
                    // e[k-1] is negligible, k<p, and s(k), ..., s(p) are not negligible (qr step).
                    kase = 3;
                }
                else if (ks == (p - 1))
                {
            ...
            case 3:
            {
                final double scale = max(max(max(max(abs(this.s[p - 1]), abs(this.s[p - 2])), abs(e[p - 2])), abs(this.s[k])), abs(e[k]));
